...
module Make (M : MonadCore) : sig
  open M

  val tp_of_tm : tm -> tp m
  ...
  module Refiner : sig
    ...
    *)
    ...
    val run_syn_rule : syn_rule -> tm m
    ...
    val run_chk_rule : chk_rule -> tp -> tm m
    ...
    val run_tp_rule : tp_rule -> tp m


    (** It is also useful to read a local term off a refinement script. *)
    val tp_rule_to_ltp : tp_rule -> Syntax.ltp m
    val chk_rule_to_ltm : chk_rule -> tp -> Syntax.ltm m
